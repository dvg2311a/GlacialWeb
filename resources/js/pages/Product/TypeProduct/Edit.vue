...
const form = useForm({
    name: props.type_product?.name ?? '',
    description: props.type_product?.description ?? '',
});
...
function submit() {
    form.put(route('type_products.update', props.type_product.id), {
        onSuccess: () => {
            Swal.fire({
                title: '¡Éxito!',
                text: 'El tipo de producto ha sido actualizado exitosamente.',
                icon: 'success',
