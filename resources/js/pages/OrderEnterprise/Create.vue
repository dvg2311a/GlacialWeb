            ...
            <div class="w-full mx-auto lg:px-8 ">
                <div class=" overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-1 ml-4 flex flex-wrap">
                        ...
                        <h1 class="text-2xl font-bold mt-1">Crear pedido a empresa</h1>
                        <p class="dark:text-white w-full mt-4">Registro de pedidos con cálculo de impuestos y totales.
                        </p>
                    </div>
                </div>
                        ...
                        <div class="border rounded p-4">
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <button type="button" @click="addProduct"
                                    class="inline-flex items-center gap-2 px-3 py-1 bg-green-600 text-white rounded">
                                    <Plus :size="16" /> Añadir
                                </button>
                            </div>
                                ...
                                <table class="min-w-full table-auto">
                                    ...
                                    <tbody>
                                        <tr v-for="(p, idx) in form.products" :key="idx" class="align-top">
                                            <td class="px-2 py-2">
                                                <select v-model="p.id" @change="recalcTotals()"
                                            ...
                                            <td class="px-2 py-2">
                                                <input v-model.number="p.boxes_received" type="number" min="0"
                                                    @input="recalcTotals()" class="w-24 rounded border-gray-300" />
                                                <InputError :message="form.errors[`products.${idx}.boxes_received`]" />
                                            </td>
                                            <td class="px-2 py-2">
                                                <input v-model.number="p.purchase_price_per_box" type="number"
                                                    step="0.01" min="0" @input="recalcTotals()"
                                                    class="w-28 rounded border-gray-300" />
                                                <InputError
                                                    :message="form.errors[`products.${idx}.purchase_price_per_box`]" />
                                            </td>
                                            <td class="px-2 py-2">
                                                <input v-model.number="p.tax_percentage" type="number" step="0.01"
                                                    min="0" @input="recalcTotals()"
                                            ...
                                            <td class="px-2 py-2">
                                                <button type="button" @click="removeProduct(idx)"
                                                    class="inline-flex items-center gap-2 px-2 py-1 bg-red-600 text-white rounded">
                                                    <Trash :size="14" />
                                                </button>
                                            </td>
                                        </tr>
