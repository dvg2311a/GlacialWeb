...
const form = useForm({
	...
	products: ((props.purchase && props.purchase.length) ? props.purchase : (props.order?.purchaseOrderDetail || [])).map(p => ({
		id: p.product_id ?? p.product?.id ?? p.id ?? '',
		boxes_ordered: p.boxes_ordered ?? 0,
		boxes_received: p.boxes_received ?? 0,
		purchase_price_per_box: p.purchase_price_per_box ?? p.purchase_price_total_per_box ?? 0,
		tax_percentage: p.tax_percentage ?? 0,
	})).length ? ((props.purchase && props.purchase.length) ? props.purchase : (props.order?.purchaseOrderDetail || [])).map(p => ({
		id: p.product_id ?? p.product?.id ?? p.id ?? '',
		boxes_ordered: p.boxes_ordered ?? 0,
		boxes_received: p.boxes_received ?? 0,
		purchase_price_per_box: p.purchase_price_per_box ?? p.purchase_price_total_per_box ?? 0,
		tax_percentage: p.tax_percentage ?? 0,
	})) : [{ id: '', boxes_ordered: 0, boxes_received: 0, purchase_price_per_box: 0, tax_percentage: 0 }],
	total_tax: props.order?.total_tax ?? 0,
	subtotal_base: props.order?.subtotal_base ?? 0,
	grand_total: props.order?.grand_total ?? 0,
})
...
function submit() {
	recalcTotals()
	form.put(route('orders_enterprises.update', props.order.id), {
		onSuccess: () => {
			Swal.fire({
				title: '¡Éxito!',
...
<template>
	<Head title="Editar Orden de Empresa" />
	<AuthenticatedLayout>
		<div class="w-full">
			<div class="w-full mx-auto lg:px-8 ">
				<div class=" overflow-hidden shadow-sm sm:rounded-lg">
					<div class="p-1 ml-4 flex flex-wrap">
						...
						<h1 class="text-2xl font-bold mt-1">Editar Pedido Empresa</h1>
						<p class="dark:text-white w-full mt-4">Editar pedido con cálculo de impuestos y totales.</p>
					</div>
				</div>
				...
				<div class="mt-6 max-w-7x5 mx-auto sm:px-6 lg:px-8">
					<form @submit.prevent="submit" class="w-full p-6 rounded-lg shadow-md flex flex-col gap-6">
										...
										<tr v-for="(p, idx) in form.products" :key="idx" class="align-top">
											<td class="px-2 py-2">
												<select v-model="p.id" @change="recalcTotals()" class="rounded border-gray-300">
													...
													<option v-for="prod in props.products || []" :key="prod.id" :value="prod.id">{{ prod.name }}</option>
												</select>
						...
						<div class="flex justify-end">
							<button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded">Actualizar Pedido</button>
						</div>
					</form>
				</div>
			</div>
